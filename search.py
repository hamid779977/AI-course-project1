...
try:
    ...
    from sets import Set as set, ImmutableSet as frozenset, functools
...
class Graph:
    ...
    def validate(self):
        for name in self.nodes:
            assert isinstance(name, string_types), str(type(name)) + ": " + str(name)
        ...
        for edge in self.edges:
            assert isinstance(edge.name, string_types), type(edge.name)
